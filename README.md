...
也可以打开 Telegram，向你的机器人发送 /help 查看完整命令说明。

## **🤖 Telegram 机器人命令**

设置 Webhook 后，机器人支持以下命令：
...
1. 卡片名称：必填，例如 KnowRoaming。
2. 手机号：选填，例如 +1 234 567 8900；输入 `-` 可跳过。
3. 保号周期：必填，输入大于 0 的整数，例如 180。
4. 到期日：必填，格式为 `YYYY-MM-DD`，例如 2026-12-31。
5. 已注册平台：选填，例如 Telegram Google OpenAI；输入 `-` 可跳过。
6. 备注 / 保号要求：选填，例如 半年发一次短信；输入 `-` 可跳过。
